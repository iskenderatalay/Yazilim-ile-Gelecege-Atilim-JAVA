...
public class baglanti {
    ...
    ArrayList<String> personelOku(){
        ...
        try{
            Statement okumaDurumObj=baglantiObj.createStatement();
    ...
    int personelGuncelle(String tc,short sicilNo,String adSoyad,String dogumTarihi,boolean medeniDurum,String gorev,boolean seyahatEngel) {
        int guncelSonuc=0;
        byte guncelMedeniDurumByte=0;
        byte guncelSeyahatEngeliByte=0;

        if(medeniDurum==true)
            guncelMedeniDurumByte=1;
        if(seyahatEngel==true)
            guncelSeyahatEngeliByte=1;

        String updateString="Update personel set sicilno=?,adsoyad=?,dogumtarihi=?,medenidurum=?,gorev=?,seyahatengeli=? where tc=?";
        try {
            PreparedStatement guncelleDurumObj=baglantiObj.prepareStatement(updateString);
            ...
            guncelleDurumObj.setByte(6,guncelSeyahatEngeliByte);
            guncelleDurumObj.setString(7,tc);

            guncelSonuc=guncelleDurumObj.executeUpdate();
            baglantiObj.commit();
